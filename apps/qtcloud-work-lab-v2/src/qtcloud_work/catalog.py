...
因为命名规则规定英文文件名与中文标题不互译。这一层是实验室自留——规格未规定。
"""
...
class Catalog:
    ...
    def find(self, query: str) -> list[Entry]:
        ...
        hits = lambda entry: {name.lower() for name in entry.names}  # noqa: E731
        exact = [entry for entry in self.entries if q in hits(entry)]
        if exact:
            return exact
        return [entry for entry in self.entries if any(q in name or name in q for name in hits(entry))]
    ...
    def unregistered(self, root: Path) -> list[Path]:
        ...
        return sorted(path for path in found if path not in known)
