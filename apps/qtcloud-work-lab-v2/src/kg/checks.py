...
规则引擎的判据是**字段**，不是一行小语法：
  ...
  - executor: rule
    path: data/index.md            # 路径存在
  - executor: rule
    absent: docs/old.md            # 路径不存在
  - executor: rule
    file: data/index.md            # 文件含这段文字（file + contains 成对）
    contains: 第二大脑
  - executor: rule
    run: test -f data/index.md     # 命令在工作区根跑，退出码为零

路径相对工作区根；写绝对路径则按绝对路径。description 可省——省了由程序按字段拼一句。
"""
