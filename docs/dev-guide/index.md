...
├── workflow.py  工作流（YAML）：严格 schema（description / steps；步骤 name / description / executor / criteria；判据 executor: rule|agent|human）
...
data/                  所有数据（工作纪律）：workflows/ 定义、tasks/（<任务>.yaml 指向工作流 + <任务>.jsonl 流水）、artifacts/{report,journal}/ 产物（按任务名）
