...
**⑦ 任务成为主界面** —— 任务就是过程的一次执行实例（`specification/process/task.md`）；在飞部分是盘上的目录（`tasks/<名字>/`：task.md（指令三段）、materials.md、log.jsonl）；契约并进了指令的「验收」，判据住在那里，记录按资产进格：报告（事件）进 `data/report/`、历史（叙事）进 `data/history/`；七格状态机决定只摆出该做的那一步；每步都记流水，报告的审查者报告与最终成果由动作生成。窗口默认开在「台面」，工作区动作退到「浏览」。
