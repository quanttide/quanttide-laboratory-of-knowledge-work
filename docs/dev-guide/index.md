...
├── task.py      任务：任务目录（在飞）、七格状态机、动作、流水；报告进 data/report、历史进 data/history
└── __main__.py  python3 -m kg

tests/test_kg.py       自带测试，31 项（装 PySide6 窗口模块则多 11 项界面冒烟），不依赖 pytest
data/                  所有数据（工作纪律）：tasks/ 在飞、report/ 事件、history/ 叙事、samples/ 判例
...
**⑦ 任务成为主界面** —— 任务就是过程的一次执行实例；在飞部分是盘上的目录（`tasks/<名字>/`：task.md（指令三段）、materials.md、contract.md、log.jsonl），记录按资产进格：报告（事件）进 `data/report/`、历史（叙事）进 `data/history/`；七格状态机决定只摆出该做的那一步；每步都记流水，报告的审查者报告与最终成果由动作生成。窗口默认开在「台面」，工作区动作退到「浏览」。
...
`contract/` 与 `dossier/` 原先各有一份 Python 对象版的结构，与程序重复。决定是**删代码、留样本**：结构只由程序拥有（同一套四段写在两处就是两份事实源）；样本搬进 `data/samples/`，判例价值留着，代码的独有能力（判据写进契约文件、`run:` 判据跑命令）没丢。
